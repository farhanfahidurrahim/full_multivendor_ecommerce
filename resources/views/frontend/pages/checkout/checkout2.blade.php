                                    ...
                                    <tbody>
                                        @if (count($shippings)>0)
                                            @foreach ($shippings as $key=>$row)
                                                <tr>
                                                    <th scope="row">{{ $row->shipping_address }}</th>
                                                    <td>{{ $row->delivery_time }}</td>
                                                    <td>${{ number_format($row->delivery_charge) }}</td>
                                                    <td>
                                                        <div class="custom-control custom-radio">
                                                            <input type="radio" id="customRadio1{{ $key }}" name="delivery_charge" value="{{ $row->delivery_charge  }}" class="custom-control-input" required>
